...
import { ArrowRight, Download, Github, Linkedin, Mail, Terminal, Trash2, Code } from 'lucide-react';
...
export const Hero: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'terminal' | 'visual'>('visual');
  ...
  const [terminalLogs, setTerminalLogs] = useState<Array<{ type: 'user' | 'system'; text: string }>>([
    { type: 'system', text: 'Varshit Jha Developer Shell v2.4 initialized.' },
  ...
  const handleCommandSubmit = (e: React.FormEvent) => {
    ...
    if (cmd === 'help') {
      ...
      newLogs.push({
        type: 'system',
        text: '[SUCCESS] Hire protocol activated! Fast-tracking to contact section...',
      });
      setTimeout(() => {
        const contactSection = document.getElementById('contact');
        if (contactSection) contactSection.scrollIntoView({ behavior: 'smooth' });
        showToast('Fast-tracking to contact form!', 'success');
      }, 600);
    } else {
      newLogs.push({
        type: 'system',
  ...
  return (
    <div className="relative overflow-hidden pt-28 pb-16 md:pt-36 md:pb-24" ref={heroContainerRef}>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          {/* Headline */}
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold tracking-tight text-[var(--color-text-primary)] leading-[1.1]">
            Hi, I'm <span className="text-[var(--color-accent-light)]">Varshit Jha</span>
          </h1>

          {/* Typing Subheadline */}
          <div className="text-xl sm:text-2xl font-mono text-[var(--color-text-secondary)] flex items-center h-8">
          ...
          <p className="text-base sm:text-lg text-[var(--color-text-secondary)] leading-relaxed max-w-xl">
            Computer Science student at Parul University building modern, scalable web applications with React, TypeScript, and clean code principles.
          </p>
        ...
        {/* Right Column: Interactive Visual & CLI Terminal Centerpiece */}
